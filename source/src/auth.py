...
def login():
    ...
    if user:
        ...
        if is_pass_correct:
            refresh = create_refresh_token(identity=user.id, expires_delta=timedelta(seconds=300))
            access = create_access_token(identity=user.id, expires_delta=timedelta(seconds=300))
            return jsonify({
                "user": {
                    "refresh": refresh,
                    "access": access,
